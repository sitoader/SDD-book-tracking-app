  ...
  return (
    <div className="space-y-3 rounded-xl border border-zinc-200/80 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="flex flex-wrap gap-2">
        {STATUS_REGISTRY.map((status) => (
          <button
            key={status.value}
            type="button"
            onClick={() => handleStatusClick(status.value)}
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm font-medium transition-colors ${activeStatusFilter === status.value ? status.activeClass : status.inactiveClass}`}
          >
